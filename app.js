...
function checkSession() {
    ...
    window.df.apis.user.getSession({
        ...
        null === a.session_id || "" == a.session_id ? (document.getElementById("login").style.display = "", logIn()) : (a = new ApiKeyAuthorization("X-Dreamfactory-Session-Token", a.session_id, "header"), window.authorizations.add("X-DreamFactory-Session-Token", a), document.getElementById("login").style.display = "none", document.getElementById("menumain").style.display = "block",document.getElementById("jobinfo").style.display = "block",document.getElementById("deptchtxt").style.display = "block",document.getElementById("wlcm").style.display = "none",document.getElementById("socialbut").style.display = "none", document.getElementById("que").style.display = "block", document.getElementById("topmenu").style.display = "block", document.getElementById("loginbg").style.cssText = "", document.getElementById("loginmain").style.display = "none", alert("Welcome back"),document.getElementById("officename").innerHTML=officename,document.getElementById("userid").innerHTML=userid,  mysocketinit(), runApp())
    }, function (a) {
...
var currentdatetimef= new Date().toISOString();
var currentdatetimeparsef= Date.parse(currentdatetimef);//cur time hash
var totaltime=currentdatetimeparsef+timeval;
 if(timeval>0)
 {filterstr="flag=0 AND office="+officename+"AND datentime<="+totaltime;}
 else
 {filterstr="flag=0 AND office="+officename;}
 
window.df.apis.localmongo.getRecordsByFilter({"table_name":"scarscollection",filter:filterstr},function(response){
...
function logIn() {
    ...
    a && c && window.df.user.login({
        ...
        document.getElementById("socialbut").style.display = "none";
...
function drvque(a) {
/////////////////
a.drvclr.forEach(function(entry){
var ofinde=entry.indexOf(officename);
if (ofinde == -1) {a.drvclr.splice(ofinde, 1);}

});
a.drvengag.forEach(function(entry){
var ofindeg=entry.indexOf(officename);
if (ofindeg == -1) {a.drvengag.splice(ofindeg, 1);}
